...
func main() {
	...
	fmt.Println(dfBagCount(rules, "shiny gold")-1)
}

func dfBagCount(rules map[string][]connection, node string) int {
	children := rules[node]
	bags := 1
	for _, c := range children {
		contained := dfBagCount(rules, c.c)
		if contained > 0 {
			bags += c.n * contained
		}
	}
...
func main_1() {
	...
	for colour := range rules {
		if colour == "shiny gold" {
			continue
		}
		if dfs(rules, colour, "shiny gold") {
...
func dfs(rules map[string][]connection, node string, search string) bool {
	if node == search {
		return true
	}
	children := rules[node]
	for _, c := range children {
		if dfs(rules, c.c, search) {
			return true
		}
	}
	return false
...
func parseInput() map[string][]connection {
	rules := map[string][]connection{}

	ss := strings.Split(input, "\n")
	for _, s := range ss {
		rule := strings.Split(s, "contain")
		root := strings.TrimSuffix(rule[0], " bags ")

		if rule[1] == " no other bags." {
			rules[root] = nil
			continue
		}
		cxns := strings.Split(rule[1], ",")
		for _, c := range cxns {
			raw := strings.Fields(c)
			n, _ := strconv.Atoi(raw[0])
			colour := fmt.Sprintf("%s %s", raw[1], raw[2])
			rules[root] = append(rules[root], connection{
				n: n,
				c: colour,
			})
		}
